...
def _make_state(
    direction=TG.SIDE_LONG, *, or_high=100.0, or_low=100.0, entry_price=100.0, shares=100
):
...
class TestStage0to1Transition:
    ...
    def test_below_stage1_target_does_not_fire(self):
        ...
        actions = TG.check_titan_grip(state=st, current_price=100.92, current_shares=100)
        assert actions == []
        assert st.stage == 0
    ...
    def test_at_stage1_target_fires(self):
        ...
        st = _make_state()
        actions = TG.check_titan_grip(state=st, current_price=100.93, current_shares=100)
...
class TestStage1FirstHarvestExit:
    ...
    def test_drop_to_anchor_runner_exit(self):
        ...
        actions = TG.check_titan_grip(state=st, current_price=100.40, current_shares=75)
        assert any(a.code == TG.ACTION_RUNNER_EXIT and a.shares == 75 for a in actions)
...
class TestStage3SecondHarvest:
    ...
    def test_stage3_fires_at_188pct(self):
        ...
        actions = TG.check_titan_grip(state=st, current_price=101.88, current_shares=75)
...
class TestStage4Runner:
    ...
    def test_runner_exits_when_stop_hit(self):
        ...
        actions = TG.check_titan_grip(state=st, current_price=anchor, current_shares=50)
        assert any(a.code == TG.ACTION_RUNNER_EXIT and a.shares == 50 for a in actions)
        assert st.stage == 3
...
class TestShortMirror:
    ...
    def test_full_short_scenario(self):
        st = _make_state(direction=TG.SIDE_SHORT, or_low=100.0, entry_price=100.0)
        # Stage 1: price <= OR_Low - 0.93% = 99.07
        actions = TG.check_titan_grip(state=st, current_price=99.07, current_shares=100)
        codes = [a.code for a in actions]
        ...
        actions = TG.check_titan_grip(state=st, current_price=98.10, current_shares=75)
        codes = [a.code for a in actions]
        assert TG.ACTION_STAGE3_HARVEST in codes
        assert st.stage == 2
...
class TestParallelAlarms:
    ...
    def test_alarm_a_and_c_both_fire_a_wins(self):
        """Same-tick A_LOSS + C2: both recorded, A wins outbound classification.
        ...
        result = S.evaluate_sentinel(
            side="LONG",
            unrealized_pnl=-501.0,  # A_LOSS fires
        ...
        assert "A_LOSS" in codes, f"A_LOSS missing from {codes}"
    ...
    def test_only_c_fires_no_full_exit(self):
        ...
        assert any(a.code == TG.ACTION_STAGE1_HARVEST for a in result.titan_grip_actions)
