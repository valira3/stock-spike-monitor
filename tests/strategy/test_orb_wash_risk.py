...
def _make_pos_and_admit(eng, *, ticker="AAPL", side="long", entry=100.0, stop=99.0, shares=100):
    rb = eng._risk.get("main")
    risk_per_share = abs(entry - stop)
    risk_dollars = risk_per_share * shares
    ticket = rb.try_admit(risk_dollars=risk_dollars, notional=entry * shares)
...
def _force_signal(eng, *, ticker="AAPL", side="long", entry=100.0):
    ...
    v9.1.124: OR levels now reflect a realistic breakout geometry.
    Long: entry > or_high (entry has broken above the OR range).
    Short: entry < or_low. Pre-v9.1.124 the fixture set
    or_high=entry+0.5 / or_low=entry-0.5 -- the synthetic "signal"
    had entry INSIDE the OR range, which the new V9124-OR-RETRACE
    gate correctly rejects. Tests that need to exercise try_enter
    must supply geometry consistent with detect_breakout's output.
    """
    ...
    if side == "long":
        or_high_val = entry - 0.5  # entry is $0.50 above OR_high
        or_low_val = entry - 1.0
        proposed_stop = entry - 1.0
    else:
        or_high_val = entry + 1.0
        or_low_val = entry + 0.5  # entry is $0.50 below OR_low
        proposed_stop = entry + 1.0
    return BreakoutSignal(
        portfolio_id="main",
        ticker=ticker,
        side=side,
        signal_bar_close_iso="test",
        signal_bar_close=entry,
        or_high=or_high_val,
        or_low=or_low_val,
...
class TestRecentLossesRecording:
    def test_losing_close_records(self):
        eng = _eng()
        pos = _make_pos_and_admit(
            eng, ticker="AAPL", side="long", entry=100.0, stop=99.0, shares=100
        )
        # Exit at $98 -> loss
    ...
    def test_winning_close_does_not_record(self):
        ...
        assert ("AAPL", "long") not in eng._recent_losses or len(
...
class TestWashRiskCounter:
    def test_entry_after_loss_same_ticker_side_increments(self):
        ...
        eng.on_exit(pos, _exits.ExitDecision(reason=_exits.EXIT_STOP, price=98.0))
    ...
    def test_entry_after_winning_close_no_increment(self):
        ...
        # Win
        eng.on_exit(pos, _exits.ExitDecision(reason=_exits.EXIT_TARGET, price=102.5))
        sig = _force_signal(eng, ticker="AAPL", side="long", entry=101.0)
        eng.try_enter(sig, equity=100_000.0)
        assert eng.wash_risk_count == 0
