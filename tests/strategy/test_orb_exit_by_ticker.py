...
def isolated_env(monkeypatch):
    ...
    # Disable the SPY-regime gate (default -40bps since v9.0.0) so tests
    # that use real market dates (2026-01-02 etc.) aren't blocked by actual
    # prior-day SPY drops when the bar archive is present locally.
...
class TestRuntimeGuards:
    ...
    def test_unknown_portfolio_returns_no_op(self, isolated_env):
        live_runtime.bootstrap()
        result = live_runtime.check_exit_by_ticker(
            portfolio_id="not_a_real_portfolio",
            ticker="AAPL",
            bar_high=110.0,
            bar_low=104.0,
            bar_close=108.0,
