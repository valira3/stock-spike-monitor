...
def test_permit_long_open_when_above_ema_and_avwap():
    r = eot.evaluate_global_permit(eot.SIDE_LONG,
                                   qqq_5m_close=400.0, qqq_5m_ema9=399.0,
                                   qqq_current_price=400.5, qqq_avwap_0930=399.5)
...
def test_permit_long_closed_when_below_avwap():
    r = eot.evaluate_global_permit(eot.SIDE_LONG,
                                   qqq_5m_close=400.0, qqq_5m_ema9=399.0,
                                   qqq_current_price=399.0, qqq_avwap_0930=399.5)
    assert r["open"] is False
    assert r["reason"] == "anchor_misaligned"


def test_permit_short_open_when_below_both():
    r = eot.evaluate_global_permit(eot.SIDE_SHORT,
                                   qqq_5m_close=399.0, qqq_5m_ema9=400.0,
                                   qqq_current_price=398.5, qqq_avwap_0930=399.5)
    assert r["open"] is True


def test_permit_data_missing_closes_gate():
    r = eot.evaluate_global_permit(eot.SIDE_LONG,
                                   qqq_5m_close=None, qqq_5m_ema9=399.0,
                                   qqq_current_price=400.0, qqq_avwap_0930=399.5)
    assert r["open"] is False
...
def test_boundary_hold_long_satisfied_with_two_outside():
    r = eot.evaluate_boundary_hold(eot.SIDE_LONG, or_high=100.0, or_low=99.0,
                                   last_n_1m_closes=[100.5, 100.7])
    assert r["hold"] is True


def test_boundary_hold_long_one_close_insufficient():
    r = eot.evaluate_boundary_hold(eot.SIDE_LONG, or_high=100.0, or_low=99.0,
                                   last_n_1m_closes=[99.9, 100.5])
...
def test_boundary_hold_long_equality_breaks_hold():
    ...
    r = eot.evaluate_boundary_hold(eot.SIDE_LONG, or_high=100.0, or_low=99.0,
                                   last_n_1m_closes=[100.5, 100.0])
    assert r["hold"] is False


def test_boundary_hold_short_mirror():
    r = eot.evaluate_boundary_hold(eot.SIDE_SHORT, or_high=100.0, or_low=99.0,
                                   last_n_1m_closes=[98.5, 98.4])
    assert r["hold"] is True


def test_boundary_hold_earliest_satisfaction_time_is_0936():
    t = eot.boundary_hold_earliest_satisfaction_et()
    assert t == time(9, 36, 0)


def test_boundary_hold_or_not_set():
    r = eot.evaluate_boundary_hold(eot.SIDE_LONG, or_high=None, or_low=None,
                                   last_n_1m_closes=[100.5, 100.7])
    assert r["hold"] is False
...
def test_entry_1_long_full_stack_passes():
    r = eot.evaluate_entry_1(
        eot.SIDE_LONG,
        permit_open=True, volume_bucket_ok=True, boundary_hold_ok=True,
        di_5m=27.0, di_1m=26.0, is_nhod_or_nlod=True,
    )
...
def test_entry_1_no_nhod_fails():
    r = eot.evaluate_entry_1(
        eot.SIDE_LONG,
        permit_open=True, volume_bucket_ok=True, boundary_hold_ok=True,
        di_5m=27.0, di_1m=26.0, is_nhod_or_nlod=False,
    )
...
def test_entry_1_di_5m_24_fails():
    r = eot.evaluate_entry_1(
        eot.SIDE_LONG,
        permit_open=True, volume_bucket_ok=True, boundary_hold_ok=True,
        di_5m=24.0, di_1m=26.0, is_nhod_or_nlod=True,
    )
    assert r["fire"] is False
...
def test_entry_1_di_5m_25_exactly_fails_strict():
    r = eot.evaluate_entry_1(
        eot.SIDE_LONG,
        permit_open=True, volume_bucket_ok=True, boundary_hold_ok=True,
        di_5m=25.0, di_1m=26.0, is_nhod_or_nlod=True,
    )
    assert r["fire"] is False
...
def test_entry_1_short_mirror_passes():
    r = eot.evaluate_entry_1(
        eot.SIDE_SHORT,
        permit_open=True, volume_bucket_ok=True, boundary_hold_ok=True,
        di_5m=28.0, di_1m=26.5, is_nhod_or_nlod=True,
    )
    assert r["fire"] is True
...
def test_entry_1_blocked_by_volume_bucket():
    r = eot.evaluate_entry_1(
        eot.SIDE_LONG,
        permit_open=True, volume_bucket_ok=False, boundary_hold_ok=True,
        di_5m=27.0, di_1m=26.0, is_nhod_or_nlod=True,
    )
    assert r["fire"] is False
...
def test_entry_2_crossing_edge_with_fresh_nhod_fires():
    r = eot.evaluate_entry_2(
        eot.SIDE_LONG,
        entry_1_active=True, permit_open_at_trigger=True,
        di_1m_prev=29.5, di_1m_now=30.5,
        fresh_nhod_or_nlod=True, entry_2_already_fired=False,
    )
    assert r["fire"] is True
...
def test_entry_2_no_fresh_nhod_fails():
    r = eot.evaluate_entry_2(
        ...
        entry_1_active=True, permit_open_at_trigger=True,
        di_1m_prev=29.5, di_1m_now=30.5,
        fresh_nhod_or_nlod=False, entry_2_already_fired=False,
    )
    assert r["fire"] is False
...
def test_entry_2_sustained_di_without_crossing_fails():
    ...
    r = eot.evaluate_entry_2(
        eot.SIDE_LONG,
        entry_1_active=True, permit_open_at_trigger=True,
        di_1m_prev=31.0, di_1m_now=32.0,
        fresh_nhod_or_nlod=True, entry_2_already_fired=False,
    )
    assert r["fire"] is False
...
def test_entry_2_never_reaches_30_fails():
    r = eot.evaluate_entry_2(
        eot.SIDE_LONG,
        entry_1_active=True, permit_open_at_trigger=True,
        di_1m_prev=27.0, di_1m_now=29.5,
        fresh_nhod_or_nlod=True, entry_2_already_fired=False,
    )
    assert r["fire"] is False
    assert r["reason"] == "no_crossing"
...
def test_entry_2_fires_only_once():
    r = eot.evaluate_entry_2(
        ...
        entry_1_active=True, permit_open_at_trigger=True,
        di_1m_prev=29.5, di_1m_now=30.5,
        fresh_nhod_or_nlod=True, entry_2_already_fired=True,
    )
    assert r["fire"] is False
...
def test_velocity_fuse_long_at_1_011_pct_fires():
    op = 100.0
    cp = op * (1 - 0.01011)
    assert eot.evaluate_velocity_fuse(eot.SIDE_LONG, current_price=cp,
                                      current_1m_open=op) is True
...
def test_velocity_fuse_long_at_exactly_1_pct_does_not_fire():
    ...
    assert eot.evaluate_velocity_fuse(eot.SIDE_LONG, current_price=cp,
                                      current_1m_open=op) is False


def test_velocity_fuse_short_at_1_5_pct_fires():
    op = 100.0
    cp = op * (1 + 0.015)
    assert eot.evaluate_velocity_fuse(eot.SIDE_SHORT, current_price=cp,
                                      current_1m_open=op) is True
...
def test_maffei_long_inside_or_lower_low_exits():
    ...
    r = eot.evaluate_maffei_inside_or(
        eot.SIDE_LONG, or_high=100.0, or_low=99.0,
        current_1m_open=100.2, current_1m_close=99.8,
        current_1m_low=99.5, current_1m_high=100.3,
        prior_1m_low=99.7, prior_1m_high=100.4,
    )
...
def test_maffei_long_inside_or_higher_low_holds():
    r = eot.evaluate_maffei_inside_or(
        eot.SIDE_LONG, or_high=100.0, or_low=99.0,
        current_1m_open=100.2, current_1m_close=99.8,
        current_1m_low=99.7, current_1m_high=100.3,
        prior_1m_low=99.5, prior_1m_high=100.4,
    )
...
def test_maffei_outside_or_no_gate():
    ...
    r = eot.evaluate_maffei_inside_or(
        eot.SIDE_LONG, or_high=100.0, or_low=99.0,
        current_1m_open=100.2, current_1m_close=100.5,
        current_1m_low=100.1, current_1m_high=100.7,
        prior_1m_low=100.0, prior_1m_high=100.4,
    )
...
def test_maffei_short_inside_or_higher_high_exits():
    r = eot.evaluate_maffei_inside_or(
        eot.SIDE_SHORT, or_high=100.0, or_low=99.0,
        current_1m_open=98.8, current_1m_close=99.2,
        current_1m_low=98.7, current_1m_high=99.5,
        prior_1m_low=98.6, prior_1m_high=99.3,
    )
    assert r["gated"] is True and r["decision"] == "EXIT"


def test_two_bar_lock_advances_on_favorable():
    s = eot.two_bar_lock_step(eot.SIDE_LONG, counter=0,
                              candle_open=100.0, candle_close=100.5)
    assert s["counter"] == 1 and s["locked"] is False
    s2 = eot.two_bar_lock_step(eot.SIDE_LONG, counter=s["counter"],
                               candle_open=100.5, candle_close=100.9)
    assert s2["counter"] == 2 and s2["locked"] is True


def test_two_bar_lock_resets_on_unfavorable():
    s = eot.two_bar_lock_step(eot.SIDE_LONG, counter=1,
                              candle_open=100.0, candle_close=99.8)
    assert s["counter"] == 0


def test_two_bar_lock_short_mirror():
    s = eot.two_bar_lock_step(eot.SIDE_SHORT, counter=1,
                              candle_open=100.0, candle_close=99.5)
    assert s["counter"] == 2 and s["locked"] is True


def test_ema_trail_long_exits_below_ema():
    assert eot.evaluate_ema_trail(eot.SIDE_LONG, candle_5m_close=99.0,
                                  ema_9_5m=100.0) is True


def test_ema_trail_long_holds_above_ema():
    assert eot.evaluate_ema_trail(eot.SIDE_LONG, candle_5m_close=100.5,
                                  ema_9_5m=100.0) is False


def test_ema_trail_short_mirror():
    assert eot.evaluate_ema_trail(eot.SIDE_SHORT, candle_5m_close=101.0,
                                  ema_9_5m=100.0) is True
...
def test_exit_reason_enum_complete():
    expected = {
        "sovereign_brake", "velocity_fuse", "forensic_stop",
        "be_stop", "ema_trail", "daily_circuit_breaker", "eod", "manual",
    }
