...
- **Partial profit at 1R** (v8.1.0, opt-in via `ORB_PARTIAL_PROFIT_AT_1R=1`): on first 1R touch, half-close at the 1R price, mark `pos.partial_taken=True`, record `partial_pnl_dollars` on the position, release half the risk-book ticket. Runner half rides to RR=2.5 target with stop at entry (BE arms on the same bar). Realized P&L at final exit = `(exit-entry) * remaining + partial_pnl_dollars`. **Paper-only** in v8.1.0 -- `broker/orders.py:partial_close_breakout` mutates paper state but does not yet emit an executor signal for live Alpaca partial sells.
...
| `ORB_PARTIAL_PROFIT_AT_1R` | `0` | If `1`, half-close at 1R touch; runner rides to RR=2.5 target with BE stop (v8.1.0+). Paper-only in v8.1.0 -- do NOT flip on live Alpaca until executor partial-close is wired. |
