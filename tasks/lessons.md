...
- **`tools/orb_backtest.py` defaults now match the live engine, not the legacy sweep.** Pre-2026-05-20 the backtest defaulted `ORB_MIN_BREAK_BPS=0` and `ORB_MAX_CONCURRENT_PER_TICKER_SIDE=0` (unlimited), but the production engine has them at `5` and `1` respectively. That gap inflated the documented Keystone morning leg by ~$23k/yr (from +$20k actual to +$37k headline). On 2026-05-20 the backtest defaults were flipped to match the engine. Result: a bare-bones Keystone run now produces production-realistic ~$20k/yr morning instead of the legacy ~$43k/yr. If you're reproducing PRE-2026-05-20 sweep numbers, you'll need to explicitly set `ORB_MIN_BREAK_BPS=0` and `ORB_MAX_CONCURRENT_PER_TICKER_SIDE=0`.
