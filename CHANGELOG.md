...
- feat (CLI): new `--export-equity PATH` flag on `python -m backtest.replay`. When set, after the per-config CSV ledgers + optional validation reports are written, the replayer writes a JSON file with the same shape as `/api/shadow_charts` — `{ "configs": { CONFIG_NAME: { equity_curve, daily_pnl, win_rate_rolling }, ... }, "as_of": "<utc iso>" }` — so dashboard-style charts can be rendered offline from any backtest run. Implemented as `backtest.replay.equity_charts_payload` + `write_equity_export`; payload computation mirrors `dashboard_server._shadow_charts_payload` (cumulative P&L per closed trade, daily P&L bucketed on `exit_ts` date, rolling 20-trade win rate with the same warm-up gate).
- feat (scheduler): nightly offline backtest. New `scripts/nightly_backtest.py` entry point and a `_run_nightly_backtest` wrapper in `trade_genius.py` wired into `scheduler_thread()` as an `("everyday", "22:00", ...)` job — 22:00 ET is post-EOD reconciliation and aligns with ~02:00 UTC year-round (within one hour during DST shifts). The script: (1) picks the most recent completed NYSE trading day via `volume_profile.is_trading_day`, (2) replays every `SHADOW_CONFIGS` entry for that day with `--validate --export-equity /data/backtest_reports/<DAY>_charts.json`, (3) captures stdout/stderr to `/data/backtest_reports/<DAY>_log.txt`, (4) writes a top-level `/data/backtest_reports/latest.json` index with per-config `match_rate / replay_only_count / prod_only_count / total_pnl / trade_count / win_rate / exit_code`, and (5) **always exits 0** so Railway never restarts the container on validation drift. Drift is surfaced through the dashboard, not the orchestrator.
- feat (backend): new `GET /api/backtest_latest` endpoint in `dashboard_server`. Reads `/data/backtest_reports/latest.json` and returns the contents directly; when the file is missing it returns a graceful placeholder `{"status": "no_data", "as_of": null, "configs": {}}` with a 200 status code so the dashboard never has to special-case 5xx. Same session-cookie auth as the rest of `/api/*`.
- feat (frontend): new "Latest backtest" card at the top of the Shadow tab, above the existing charts subsection. Headline shows aggregate match-rate (color-coded green ≥0.95 / yellow 0.90–0.95 / red <0.90) plus best/worst config by total P&L. The card is collapsible — click / Enter / Space toggles — and expands a per-config table with `match_rate` / `replay_only` / `prod_only` / `total_pnl` / `trade_count` / `win_rate` for all 7 `SHADOW_CONFIGS`. Default state: expanded on desktop, collapsed on ≤720 px mobile, matching the existing Charts card. Polling is tab-aware (60 s loop, only fires while the Shadow tab is active) and an immediate fetch is triggered on tab activation. Module-scoped — does not touch the v5.4.1 Chart.js click/tooltip handlers.
- docs: refresh `ARCHITECTURE.md` to v5.5.0; new §22 *Nightly backtest* documents the scheduler entry, the `/api/backtest_latest` endpoint and response shape, the Shadow tab "Latest backtest" card, and the `--export-equity` CLI flag. Header version v5.4.1 → v5.5.0; intro now covers the nightly cadence; §16.5 release table extended with a v5.5.0 row.
- docs: regenerate `trade_genius_algo.pdf` from the refreshed `ARCHITECTURE.md` via `scripts/build_algo_pdf.py`. Cover page now reads **v5.5.0**.
- CI guard: `BOT_VERSION` bumped to `5.5.0` (matches this heading; the version-bump-check workflow gates on both).
- tests: 4 new smoke tests — `test_v550_export_equity_flag`, `test_v550_nightly_backtest_writes_latest`, `test_v550_backtest_latest_endpoint`, `test_v550_backtest_latest_endpoint_missing`.
