...
v3.5.0 paper-only: all commands route to the paper portfolio. (TradersPost / Robinhood / Gmail/IMAP surfaces were removed — see CHANGELOG.)
...
| `/trade_log` | — | Last 10 rows from the persistent paper trade log. |
...
| Command | Args | Description |
|---------|------|-------------|
| `/reset` | — | Interactive reset with an inline confirm button (60-second expiry). Resets paper to $100,000 after confirmation. |
...
Admin
  /reset            Reset paper portfolio
  /retighten        Force-cap stops
  /near_misses      Recent declined breakouts
  /trade_log        Last 10 log rows
