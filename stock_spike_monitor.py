...
```
Keys:
  indices_lines  — list of formatted index strings
  indices_str    — single-line summary for Claude prompts
  sector_lines   — list of formatted sector strings
  sector_str     — top 3 sectors for Claude prompts
  fg_val         — Fear & Greed value (int)
  fg_label       — Fear & Greed label (str)
  fg_str         — formatted F&G string
  vix            — VIX price (float)
  crypto_lines   — list of formatted crypto strings
  crypto_str     — top 3 crypto for Claude prompts
  movers_str     — top gainers/losers from watchlist for Claude
  now_label      — formatted date string
  session        — trading session string
"""
def _fetch_indices():
