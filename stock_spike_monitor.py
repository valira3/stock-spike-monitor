...
BOT_VERSION = "1.19"
RELEASE_NOTES = [
    "1.19 — Cash Account: removed PDT tracker, added T+1 settlement tracking for cash account.",
...
def _log_exit_drift(ticker, reason="send_failed"):
    """Log drift when an EXIT fails on TradersPost side."""
    sp = tp_state.setdefault("shadow_portfolio",
                             _default_shadow_portfolio())
    now = datetime.now(CT)
    warn = {
    ...
    if len(sp["drift_warnings"]) > 50:
        sp["drift_warnings"] = sp["drift_warnings"][-50:]
    tp_log(
        f"⚠️ DRIFT: EXIT {ticker} ({reason}) — "
        "paper sold, TradersPost may still hold"
    )
    save_paper_state()
            ...
            if user_config.get("trading_mode") == "shadow":
                try:
                    ...
                    if tp_result:
                        ...
                        _log_exit_drift(ticker, "send_failed")
                except Exception as e:
