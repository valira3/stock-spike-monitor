...
- **Strategy-rule development (added v9.0.0).** Before adding a new admission filter, exit rule, or day-gate, read `.claude/rules/strategy.md`. The 12 rules in that doc are hard-won lessons from the v8.3.x → v9.0.0 arc: reproduce baseline, defaults-ON keep tests passing, quarterly CV mandatory, plateau-test the winner, fence don't globalize, break vs continue in multi-window loops, bootstrap includes zero days, conservative ≠ full skip, audit every override, document falsified theories, R6 defenses conflict with v9 chase, afternoon doesn't work. Pair with `.claude/skills/backtest-research-methodology/SKILL.md` for the 7-phase research loop.
