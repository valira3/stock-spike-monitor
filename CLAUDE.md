...
- **Broker fire**: Main goes through `callbacks.execute_entry` (legacy path). Val/Gene route through `executors/base.py:fire_long`/`fire_short` (always-independent since v9.1.128).
- **Per-portfolio exit pass** (v9.1.127): `engine/scan.py:_v10_per_portfolio_exit_pass` runs every scan tick for non-main portfolios — iterates each adapter's open positions, calls `check_exit_by_ticker(portfolio_id=pid)`, fires close via `_v10_dispatch_executor_fire(reduce_only=True)` on full exit and `_v10_dispatch_executor_partial_close` on 1R partial.
...
- **Always-independent portfolios (v8.3.23 introduced as default; v9.1.128 made it the only mode).** Each portfolio runs its own `OrbEngine.try_enter` and dispatches entries via `engine/scan.py:_v10_dispatch_executor_fire` → `executor.fire_long`/`fire_short`. The legacy bus listener `executors/base.py:_on_signal` unconditionally skips `ENTRY_*`/`EXIT_*`/`PARTIAL_EXIT_*` to prevent double-fire; the `ORB_PORTFOLIO_FIRE` env flag was removed (was a kill-switch for the deprecated mirror mode). EXIT signals fire per-portfolio via `engine/scan.py:_v10_per_portfolio_exit_pass` (v9.1.127); EOD safety-net sweep at 15:57 ET still arrives via bus `EOD_CLOSE_ALL` (v9.1.126).

- **Keystone backtest (added v9.1.35).** When the operator asks to "run keystone", "verify baseline", "re-run the benchmark", or "check if results still hold", invoke `.claude/skills/keystone-backtest/SKILL.md`. It encodes the exact `orb_backtest.py` + `afternoon_backtest.py` commands, the `--year-prefix 20` gotcha, pkl cache behavior, and anti-patterns (T5 blocklist, missing cooldown, wrong EOD entry bucket). After any strategy lever is promoted to production, update Keystone and `KEYSTONE` dict in `tools/dashboard_analysis.py`.
...
# NOTE: AFT_ENTRY_BUCKET=900 (15:00 ET), AFT_EXIT_BUCKET=958 (15:58 ET, v9.1.109).
```
...
| r17 EOD fence | ORCL/AAPL/MSFT/AVGO/TSLA long, ORCL/NFLX/AAPL/MSFT/TSLA short | Per-ticker sweep; TSLA added v9.1.113 (+$3,930/yr). GOOG/NVDA remain excluded (tested negative). |
...
- Portfolios are always independent (v9.1.128). Val/Gene fire their own entries (`engine/scan.py:_v10_dispatch_executor_fire`), exits (`_v10_per_portfolio_exit_pass`), and partials (`_v10_dispatch_executor_partial_close`); the legacy bus-mirror path was removed. The `ORB_PORTFOLIO_FIRE` env var is no-op — safe to drop from Railway.
