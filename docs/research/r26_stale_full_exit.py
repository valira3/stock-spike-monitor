"""Round 26: stale full-position exit for un-partialed cohort (2026-05-18).

Today's Val AVGO short MFE=0.17R, partial never fired. v9.1.129
runner_eod_prep=14:00 lever was no-op because it requires partial_taken.
The legacy sentinel A used to cover this case at the Main level, but
v9.1.128 made portfolios independent and Val/Gene no longer have that
safety net.

This sweep tests a complementary lever: force-close any position whose
1R partial has NOT fired by time T, optionally floored on MFE-in-R so
we don't yank trades that came close to triggering partial.

Hypothesis: shaves giveback on the not-yet-partialed cohort that
afternoon-chops back into stop or EOD-cutoff loss. Magnitude likely
small (R25 stale-PARTIAL was only +$707). If this is also marginal,
we accept that the v9.1.128-removed sentinel A safety net cost is
fundamentally not recoverable via simple time/MFE rules.

Variants:
  1. Force-close at T (no MFE floor) for T in {13:00,...,15:30}
  2. Force-close at 14:00 with floor R in {0.3, 0.5, 0.7}
     (only close trades that never even got to floor*R)
  3. Force-close at 15:00 with floor R in {0.3, 0.5, 0.7}
  4. Stack winner on top of R21 14:00 runner_eod_prep
"""
...
# Production-equivalent (with R21 14:00 runner_eod_prep enabled, since
# it's live). We're testing R26 stacked on top of the current prod
# config.
PROD = {**BASE, "ORB_RUNNER_EOD_PREP_ET": "14:00"}

THEORIES = [
    ("R26_baseline_prod_R21_only", {}),
    # No-floor sweeps (close ALL un-partialed at T).
    ("R26_full_1300_floor0", {"ORB_STALE_FULL_EXIT_ET": "13:00"}),
    ("R26_full_1330_floor0", {"ORB_STALE_FULL_EXIT_ET": "13:30"}),
    ("R26_full_1400_floor0", {"ORB_STALE_FULL_EXIT_ET": "14:00"}),
    ("R26_full_1430_floor0", {"ORB_STALE_FULL_EXIT_ET": "14:30"}),
    ("R26_full_1500_floor0", {"ORB_STALE_FULL_EXIT_ET": "15:00"}),
    ("R26_full_1530_floor0", {"ORB_STALE_FULL_EXIT_ET": "15:30"}),
    # 14:00 cutoff, vary MFE floor.
    ("R26_full_1400_floor03", {"ORB_STALE_FULL_EXIT_ET": "14:00", "ORB_STALE_FULL_EXIT_MFE_FLOOR_R": "0.3"}),
    ("R26_full_1400_floor05", {"ORB_STALE_FULL_EXIT_ET": "14:00", "ORB_STALE_FULL_EXIT_MFE_FLOOR_R": "0.5"}),
    ("R26_full_1400_floor07", {"ORB_STALE_FULL_EXIT_ET": "14:00", "ORB_STALE_FULL_EXIT_MFE_FLOOR_R": "0.7"}),
    # 15:00 cutoff, vary MFE floor.
    ("R26_full_1500_floor03", {"ORB_STALE_FULL_EXIT_ET": "15:00", "ORB_STALE_FULL_EXIT_MFE_FLOOR_R": "0.3"}),
    ("R26_full_1500_floor05", {"ORB_STALE_FULL_EXIT_ET": "15:00", "ORB_STALE_FULL_EXIT_MFE_FLOOR_R": "0.5"}),
    ("R26_full_1500_floor07", {"ORB_STALE_FULL_EXIT_ET": "15:00", "ORB_STALE_FULL_EXIT_MFE_FLOOR_R": "0.7"}),
    # 13:30 + floor (earlier cutoff, more aggressive)
    ("R26_full_1330_floor05", {"ORB_STALE_FULL_EXIT_ET": "13:30", "ORB_STALE_FULL_EXIT_MFE_FLOOR_R": "0.5"}),
]


def _run_morning(tid, overrides, account):
    out = REPO / "results" / "r26" / f"{tid}_acct{account}" / "morning"
    if (out / "summary.json").exists():
        return out
    out.mkdir(parents=True, exist_ok=True)
    env = {**os.environ, **PROD, **overrides, "ORB_ACCOUNT": str(account)}
    try:
        subprocess.run(
...
def main():
    print(f"R26: {len(THEORIES)} variants x 2 accounts (Val + Main)", flush=True)
    t0 = time.time()
    ...
    print("R26 STALE-FULL-EXIT RANKING (sorted by Val combined annualized $)")
    print("=" * 130)
