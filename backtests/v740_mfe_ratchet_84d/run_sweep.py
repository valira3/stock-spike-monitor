#!/usr/bin/env python3
"""v7.4.0 MFE-Ratchet 83-day SIP sweep: 3 fracs vs v7.3.0 baseline.

Stacked on v7.3.0 stop-hysteresis (BARS=2, DEEP_FRAC=0.0075).
Sweep V740_MFE_RATCHET_FRAC: 0.3 (loose), 0.5 (balanced), 0.7 (tight).
Expected wall: ~30-36 min (3 variants \u00d7 ~12 min, 2 workers each).
"""
...
ISOLATE = Path("/tmp/v740_isolate_84d")
...
# Three variants. All include v7.3.0 hysteresis + prod parity.
_BASE = {
...
VARIANTS: list[tuple[str, str, dict, str]] = [
    ("v740", "frac_03", {**_BASE, "V740_MFE_RATCHET_FRAC": "0.3"}, "frac03"),
    ("v740", "frac_05", {**_BASE, "V740_MFE_RATCHET_FRAC": "0.5"}, "frac05"),
    ("v740", "frac_07", {**_BASE, "V740_MFE_RATCHET_FRAC": "0.7"}, "frac07"),
]
...
def main() -> int:
    ...
    state: dict = {
        ...
        "version": "v7.4.0",
        "corpus": "83d_sip_v707",
        "lever": "Lever #3 MFE-ratchet trail",
    ...
    for sweep_dir, variant_name, extras, isolate_subdir in VARIANTS:
        ...
        print(f"  V740_MFE_RATCHET_FRAC={extras['V740_MFE_RATCHET_FRAC']}", flush=True)
        ...
        with cf.ProcessPoolExecutor(max_workers=WORKERS) as ex:
            ...
            for fut in cf.as_completed(futures):
                ...
                if done % 10 == 0 or done == len(tasks):
                    ...
                    state["progress"] = {
                        "done": done, "total": len(tasks), "ok": ok_so_far,
                        "pnl": round(pnl_so_far, 2), "elapsed_min": round(elapsed, 1),
                    }
                    write_progress(state)
        ...
        summary = {
            "sweep": sweep_dir, "variant": variant_name, "version": "v7.4.0",
            "corpus": "83d_sip_v707", "extras": extras,
            "days_total": len(dates), "days_run": len(tasks), "days_ok": ok_count,
            "net_pnl_83d": round(net_pnl, 2), "entry_count": entries,
            "pnl_pair_count": pnl_pairs, "wins": wins, "losses": losses,
            "win_rate": round(wr, 2), "wall_min": wall_min,
        }
        ...
        write_progress(state)
        print(
            f"\n=== DONE {sweep_dir}/{variant_name}: ok={ok_count}/{len(dates)} "
