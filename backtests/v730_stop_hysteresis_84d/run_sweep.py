#!/usr/bin/env python3
"""v7.3.0 stop-hysteresis 83-day SIP sweep vs $1,665 clean baseline.

Lever #1: stop-price hysteresis. Requires N=2 consecutive 1m closes beyond stop
before firing (vs single-tick legacy). Deep-breach override at 0.75% gap.

Settings: prod parity (LONG=30/SHORT=30, vol gate off) + V730 hysteresis knobs.
Expected wall: ~10-12 min (2 workers, warm bar cache).
"""
...
ISOLATE = Path("/tmp/v730_isolate_84d")
...
# v7.3.0 hysteresis + prod-parity settings
VARIANTS: list[tuple[str, str, dict, str]] = [
    ("v730", "stop_hysteresis", {
        # Lever #1: stop-price hysteresis
        "V730_STOP_HYSTERESIS_ENABLED": "1",
        "V730_STOP_HYSTERESIS_BARS": "2",
        "V730_STOP_DEEP_FRAC": "0.0075",
        # Prod parity (matches live env as of 2026-05-04)
...
def main() -> int:
    ...
    state: dict = {
        ...
        "version": "v7.3.0",
        "corpus": "83d_sip_v707",
        "lever": "Lever #1 stop-price hysteresis",
    ...
    for sweep_dir, variant_name, extras, isolate_subdir in VARIANTS:
        v_start = time.time()
        state["current_variant"] = f"{sweep_dir}/{variant_name}"
        write_progress(state)
        print(f"\n=== START {sweep_dir}/{variant_name} ===", flush=True)
        print(f"  extras: {extras}", flush=True)

        out_root = ROOT / sweep_dir / variant_name
        ...
        with cf.ProcessPoolExecutor(max_workers=WORKERS) as ex:
            ...
            for fut in cf.as_completed(futures):
                ...
                if r["ok"]:
                    empty_streak = 0
                    pnl_str = f"pnl={r.get('net_pnl', 0):.2f}"
                    entries_str = f"entries={r.get('entries', 0)}"
                    print(f"  OK  {r['date']} {entries_str} {pnl_str}", flush=True)
                else:
                    empty_streak += 1
                ...
                if done % 5 == 0 or done == len(tasks):
                    ...
                    print(
                        f"  PROGRESS {done}/{len(tasks)} ok={ok_so_far} "
                        f"pnl={pnl_so_far:.2f} streak={empty_streak} "
                ...
                if empty_streak > 5:
                    print(f"  ABORT: empty_streak={empty_streak} > 5, killing run!", flush=True)
                    for f in futures:
                        f.cancel()
                    aborted = True
                    break
        ...
        if aborted:
            print("Run aborted due to excessive failures. Check stderr above.", flush=True)
            state["aborted"] = True
            write_progress(state)
            return 1
        ...
        write_progress(state)
        print(
            f"\n=== DONE {sweep_dir}/{variant_name}: "
            f"ok={ok_count}/{len(dates)} pnl={net_pnl:.2f} "
            f"wr={wr:.1f}% wall={wall_min}min ===",
            flush=True,
        )
